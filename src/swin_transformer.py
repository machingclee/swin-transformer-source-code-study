...
from src.utils.drop_path import DropPath
...
def window_partition(x, window_size):
    ...
    # windows = rearrange(
    #     x, 
    #     "b c (s1 h_patches) (s2 w_patches) -> (b h_patches w_patches) s1 s2 c", 
    #     s1=window_size, 
    #     s2=window_size
    # )
...
def window_reverse(windows, window_size, H, W):
    ...
    # x = rearrange(
    #     windows , 
    #     "(b h_patches w_patches) s1 s2 c -> b (s1 h_patches) (s2 w_patches) c", 
    #     s1=window_size,
    #     s2=window_size
    # )
...
class PatchEmbedding(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_channels=3, embed_dim=96, norm_layer=nn.LayerNorm):
        super(PatchEmbedding, self).__init__()
        h, w = img_size, img_size
        patches_resolution = [h // patch_size, w // patch_size]
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        assert h == self.img_size and w == self.img_size, f"input images size must bf of {self.img_size}x{self.img_size}"
        x = self.proj(x) # shape [b, embed_dim, n_patches_x, n_patches_y]
        x = rearrange(x, "b c patches_x patches_y -> b (patches_x patches_y) c")
        x = self.norm(x)
        return x
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=100,
        ...
        # stochastic depth
        dpr = [x for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
